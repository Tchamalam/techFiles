			...
			<div class="col-md-9">
				...
				<div class="panel panel-default">
					...
					if(isset($_POST['gonder'])){
						...
						$son_id=mysql_fetch_object(mysql_query("select * from iletisim order by ids desc"));
						$idal=$son_id->id;
						$idal=$idal+1;
						$baslik   = $_POST["basliktr"];
						$sor = mysql_query("SELECT * FROM iletisim WHERE baslik='$baslik'");
							...
							if($baslik == ""){
								...
								if($_FILES["dosya"]["size"] > 0){
									...
									if ($yukle){
										...
										$ekle =  mysql_query("INSERT INTO iletisim (id, baslik, resim, yazi, tarih, dil) VALUES ('$idal', '$baslik',  '$yeniresimadi', '$yazi', '$tarih', '$diller->id')");
										 }
									...
									$yazi 	  = $_POST["yazi".$diller->links];
									$yazi=str_replace("'", "`", $yazi);
									$ekle2 = mysql_query("INSERT INTO iletisim (id, baslik, yazi, tarih,dil) VALUES ('$idal', '$baslik', '$yazi', '$tarih', '$diller->id')");
									}
							...
							<div class="tab-pane <?php if($j==1){ echo "active"; } ?> fade in" id="<?php echo $diller2->links?>">
								...
								<fieldset>
									...
									<div class="form-group">
										...
										<div class="col-lg-10">
										<input type="text" class="form-control" id="inputEmail" name="baslik<?php echo $diller2->links?>" placeholder="SSS Başlığı">
										</div>
									</div>
