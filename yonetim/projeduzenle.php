				...
				<div class="panel panel-default">
					...
					if(isset($_POST['gonder'])){
						...
						if($_FILES["dosya"]["size"] > 0){
							...
							$whitelist = array('jpg', 'png', 'pdf', 'jpeg', 'pdf'); #whitelist örneği
							...
							if ($yukle){
								...
								while($diller=mysql_fetch_object($sql_dil)){
									...
									$sqlsay=mysql_num_rows(mysql_query("select * from projeler where baslikseo='".$baslikseo."' and dil='".$_SESSION["dil"]."'"));
									if ($sqlsay>1)
									{
									$baslikseo = $baslikseo."-".$id;
									}
									$issahibi = $_POST["issahibi".$diller->links];
									$yazi = $_POST["yazi".$diller->links];
									$kesif = $_POST["kesif".$diller->links];
									$tarih = $_POST["tarih".$diller->links];
								$ekle =  mysql_query("UPDATE projeler SET katid='$kat', hizmetid='$hizmetid', baslik='$baslik', issahibi='$issahibi', kesif='$kesif', tarih='$tarih', baslikseo='$baslikseo', public='$public', resim='$yeniresimadi', kisa_yazi='$kisa_yazi', yazi='$yazi' WHERE id='$id' and dil='".$diller->id."'");
								}
								if($ekle){
									echo '<div class="alert alert-success">Ürün Başarıyla Düzenlendi</div>';
									header("Refresh:3;url=projeler.php");
								}else{
							...
							$ekle2 = mysql_query("UPDATE projeler SET katid='$kat', hizmetid='$hizmetid', public='$public', baslik='$baslik', issahibi='$issahibi', kesif='$kesif', tarih='$tarih', baslikseo='$baslikseo', kisa_yazi='$kisa_yazi', yazi='$yazi' WHERE id='$id' and dil='".$diller->id."'");
								}
								
							if($ekle2){

								echo '<div class="alert alert-success">Proje Başarıyla Düzenlendi</div>';
								header("Refresh:3;url=projeler.php");
					...
					$gelen = mysql_fetch_assoc(mysql_query("SELECT * FROM projeler WHERE id='$id'"));

					$resid = mysql_real_escape_string($_GET['resid']);

					if($resid > 0):

						$resim = mysql_fetch_assoc(mysql_query("SELECT * FROM projeler WHERE id='$resid'"));
						$delimg = $resim['resim'];
						if(file_exists("../$delimg")){
							unlink("../$delimg");
							mysql_query("UPDATE projeler SET resim='' WHERE id='$resid'");
							header("Location:projeduzenle.php?id=".$resid);
						}

					endif;
					if($pdfid > 0):

						$pdfs = mysql_fetch_assoc(mysql_query("SELECT * FROM projeler WHERE id='$pdfid'"));
						$delpdf = $pdfs['pdf'];
						if(file_exists("../$delpdf")){
							unlink("../$delpdf");
							mysql_query("UPDATE projeler SET pdf='' WHERE id='$pdfid'");
							header("Location:projeduzenle.php?id=".$pdfid);
						}
					...
					<form class="form-horizontal" method="POST" action="<?php $PHP_SELF?>" enctype="multipart/form-data">
						...
						<div id="myTabContent" class="tab-content">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-lg-8">
                                <a href="../<?=$gelen["resim"]?>"><img src="../<?php echo $gelen["resim"];?>" width="150" style="float:left;display: block;"></a>
                                <a href="projeduzenle.php?resid=<?php echo $id;?>" class="col-lg-3">Resmi Sil</a>
                                </div>
                            </div>									
                            <?php endif; ?>
                            ...
                            <div class="form-group">
                                ...
                                <select name="kat" class="form-control">
                                    ...
                                    <?php
                                        $kategori = mysql_query("select * from projeler where katid='0' and dil='1' order by ordernum asc");
                            ...
                            <div class="form-group">
                                <label for="inputEmail" class="col-lg-2 control-label">Hizmet Kategorisi</label>
                                <div class="col-lg-10">
                                <select name="hizmetid" class="form-control">
                                    <option value="0">Seçiniz</option>
                                    <?php
                                        $hizmetkategori = mysql_query("select * from sayfa where katid='2' and dil='1' order by ordernum asc");
                                        while($hizmetkat = mysql_fetch_array($hizmetkategori)):
                                    ?>
                                    <option value="<?php echo $hizmetkat['id'];?>" <?php if($gelen["hizmetid"] == $hizmetkat['id']): echo "selected"; endif; ?>><?php echo $hizmetkat['baslik'];?></option>
	                                <? endwhile; ?>
                                </select>
                                </div>
                            </div>
							...
							while($diller2=mysql_fetch_object($sql_dil2)){
							  $syf_kyt=mysql_fetch_object(mysql_query("select * from projeler where id='$id' and dil='".$diller2->id."'"));
							$j++;
							?>
							<div class="tab-pane <?php if($j==1){ echo "active"; } ?> fade in" id="<?php echo $diller2->links?>">
								...
								<fieldset>
									...
									<div class="form-group">
										<label for="inputEmail" class="col-lg-2 control-label">Bölge</label>
										<div class="col-lg-10">
										<input type="text" class="form-control" id="inputEmail" name="issahibi<?php echo $diller2->links?>" value="<?php echo $syf_kyt->issahibi;?>">
										</div>
									</div>
									<div class="form-group">
										<label for="inputEmail" class="col-lg-2 control-label">Tarih</label>
										<div class="col-lg-10">
										<input type="text" class="form-control" id="inputEmail" name="tarih<?php echo $diller2->links?>" value="<?php echo $syf_kyt->tarih;?>">
										</div>
									</div>
									<div class="form-group">
										<label for="inputEmail" class="col-lg-2 control-label">Yüzölçümü</label>
										<div class="col-lg-10">
										<input type="text" class="form-control" id="inputEmail" name="kesif<?php echo $diller2->links?>" value="<?php echo $syf_kyt->kesif;?>">
										</div>
									</div>
									...
									<div class="form-group">
										<div class="col-lg-12">
											<button name="gonder" type="submit" class="btn btn-default">Proje Düzenle</button>
										</div>
									</div>
