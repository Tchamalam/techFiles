<?php
	...
	$syf = mysql_fetch_assoc(mysql_query("SELECT * FROM sss WHERE id='$id'"));
?>
<!DOCTYPE html>
<html class="no-js">
<head>
    ...
    function sor(id){
        ...
        if(soru == true){
            window.location.href='sssresimsil.php?id='+id;
        }else{
            return false;        
        }
...
<body>
				...
				<div class="panel panel-default">
					...
					<div class="panel-body">
					    ...
					    <div id="files" class="files">
                            <?php
                                if(isset($_POST['tumunu_sil'])){
                                    ...
                                    foreach($resim_sil as $deger){
                                        $resim_name = mysql_fetch_assoc(mysql_query("SELECT * FROM sssresimler WHERE id='$deger'"));
                                        $resim_name2 = "../".$resim_name['resim'];
                                        if(file_exists($resim_name2)){
                                            unlink("$resim_name2");
                                            mysql_query("DELETE FROM sssresimler WHERE id='$deger'");
                                        }
                                    }
                                }                            
                            ?>
					    	...
					    	<?php
					    		$resim = mysql_query("SELECT * FROM sssresimler WHERE resid='$id' ORDER BY ordernum ASC");
	...
	$(function () {
	    ...
	    var url = window.location.hostname === 'blueimp.github.io' ?
                '//jquery-file-upload.appspot.com/' : 'sssdosyaYukle.php?id=<?php echo $id;?>';
