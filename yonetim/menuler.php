		...
		<div class="row">
			...
			<div class="col-md-9">
				<div class="panel panel-default">
					...
					<h3 class="panel-title">Sitede Bulunan Menüler</h3>
					</div>
					...
					if($id > 0){
						...
						$dyr = mysql_fetch_assoc(mysql_query("SELECT * FROM menuler WHERE id='$id'"));
						$dyr_adi = $dyr["resim"];
						if(file_exists("../$dyr_adi")){
							@unlink("../$dyr_adi");
						}

						$sil = mysql_query("DELETE FROM menuler WHERE id='$id'");
						
						if($sil){

							$resimc = mysql_query("SELECT * FROM menuresimler WHERE resid='$id'");
							while ($resim_res = mysql_fetch_array($resimc)) {
								$resim = $resim_res["resim"];
								unlink("../$resim");
							}

							$sil = mysql_query("DELETE FROM menuresimler WHERE resid='$id'");
					...
					?>
						...
						<li>
						<div class="col-md-8"><b><u>Menü Başlığı</u></b></div>
						<div class="col-md-4"><b><u>İşlemler</u></b></div>
						  ...
						  $pr = mysql_query("SELECT * FROM menuler ORDER BY ordernum ASC LIMIT $goster,$limit");
						  ...
						  <div class="col-md-4"><a href="menuduzenle.php?id=<?php echo $pr_getir['id']?>">Düzenle</a> - <a href="menuler.php?id=<?php echo $pr_getir['id']?>" onClick="return confirm('Bu Menüyü Silmek İstediğinizden Emin misiniz?')">Sil</a></div>
						  </li>
						  ...
						  <ul class="pagination">
		                    ...
		                    if($sayfa > 1){
		                        $onceki = $sayfa - 1;
		                        echo "<li><a href='menuler.php?s=$onceki' class'prev'><i class='fa fa-chevron-left'></i></a></li>";
		                    }else{
		                    ...
		                    for($i= $sayfa - $gorunen; $i < $sayfa + $gorunen + 1; $i++){
		                        ...
		                        if($i > 0 and $i <= $sayfa_sayisi){
		                                ...
		                                echo "<li><a href='menuler.php?s=$i'>$i</a></li>";
		                            }
		                        }
		                    }
		                    ...
		                    if($sayfa != $sayfa_sayisi){
		                        $sonraki = $sayfa +1;
		                        echo "<li><a href='menuler.php?s=$sonraki' class='next'><i class='fa fa-chevron-right'></i></a></li>";
		                    }else{
