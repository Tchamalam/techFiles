		...
		<div class="row">
			...
			<div class="col-md-9">
				...
				<div class="panel panel-default">
					...
					<h3 class="panel-title">Müşteri Düzenle</h3>
					...
					if(isset($_POST['gonder'])){
						...
						if($_FILES["dosya"]["size"] > 0){
							...
							$duyuru = mysql_fetch_assoc(mysql_query("SELECT * FROM musteriler WHERE id='$id'"));
							$duyuru_adi = $duyuru["resim"];

							if(file_exists("../$duyuru_adi")){
								@unlink("../$duyuru_adi");
							}
							...
							if ($yukle){
								...
								$resim = mysql_query("SELECT * FROM musteriler WHERE id='$id'");
								...
								$sqlsay=mysql_num_rows(mysql_query("select * from musteriler where baslikseo='".$baslikseo."' and dil='".$_SESSION["dil"]."'"));
								if ($sqlsay>1)
								{
								...
								$guncelle = mysql_query("UPDATE musteriler SET baslik='$baslik', baslikseo='$baslikseo', lefts='$left', top='$top', resim='$yeniresimadi', yazi='$yazi', public='$public' WHERE id='$id' and dil='".$diller->id."'");
								}
							...
							$yazi = $_POST["yazi".$diller->links];
							$yazi=str_replace("'", "`", $yazi);
							$guncelle = mysql_query("UPDATE musteriler SET baslik='$baslik', baslikseo='$baslikseo', lefts='$left', top='$top', yazi='$yazi', public='$public' WHERE id='$id' and dil='".$diller->id."'");
							}
							if($guncelle){
					...
					$gelen = mysql_fetch_assoc(mysql_query("SELECT * FROM musteriler WHERE id='$id'"));
					...
					<form class="form-horizontal" method="POST" action="<?php $PHP_SELF?>" enctype="multipart/form-data">
						...
						<?php if($gelen['resim'] != ""){ ?>
                            ...
                            <div class="form-group">
                                <label for="inputEmail" class="col-lg-2 control-label">Left Değeri</label>
                                <div class="col-lg-10">
                                <input type="text" class="form-control" id="inputEmail" name="left" placeholder="" value="<?php echo $gelen["lefts"]?>">
                                </div>
                            </div>
                            <div class="form-group">
                                <label for="inputEmail" class="col-lg-2 control-label">Top Değeri</label>
                                <div class="col-lg-10">
                                <input type="text" class="form-control" id="inputEmail" name="top" placeholder="" value="<?php echo $gelen["top"]?>">
                                </div>
                            </div>
							<?php
							$j=0;
							...
							$j++;
							  $dyr_kyt=mysql_fetch_object(mysql_query("select * from musteriler where id='$id' and dil='".$diller2->id."'"));?>
							<div class="tab-pane fade <?php if($j==1){ echo "active"; } ?> in" id="<?php echo $diller2->links?>">
								...
								<fieldset>
									...
									<div class="form-group">
										...
										<div class="col-lg-10">
										<input type="text" class="form-control" id="inputEmail" name="baslik<?php echo $diller2->links?>" placeholder="Temsilci Başlığı" value="<?php echo $dyr_kyt->baslik?>">
										</div>
									</div>
					...
					if($resimid > 0){
						...
						$resim = mysql_query("SELECT * FROM musteriresimler WHERE id='$resimid'");
						...
						$sil = mysql_query("DELETE FROM musteriresimler WHERE id='$resimid'");
					...
					<div class="row">
				    	...
				    	$rs = mysql_query("SELECT * FROM musteriresimler WHERE resid='$id' ORDER BY ordernum ASC");
				        ...
				        <div class="col-xs-3" id="listItem_<?php echo $rs_getir['id']; ?>">
				            ...
				            <center><a href="musteriduzenle.php?id=<?php echo $id?>&resimid=<?php echo $rs_getir['id']?>"><span class="glyphicon glyphicon-remove"></span></a></center>
		...
		$(function() {
		  ...
		  $("#test-list").sortable({
		    ...
		    update:function(){
		      ...
		      $.post("musteriresimsiralaislem.php",posted,function(e){
