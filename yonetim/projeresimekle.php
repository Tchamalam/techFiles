<?php
	...
	$syf = mysql_fetch_assoc(mysql_query("SELECT * FROM projeler WHERE id='$id'"));
?>
<!DOCTYPE html>
<html class="no-js">
<head>
    ...
    function sor(id){
        ...
        if(soru == true){
            window.location.href='projeresimsil.php?id='+id;
        }else{
            return false;        
        }
...
<body>
		...
		<div class="row">
			...
			<div class="col-md-9">
				<div class="panel panel-default">
					...
					<h3 class="panel-title"><b><?php echo $syf['baslik']?></b> Projeye Resim(ler) Ekle</h3>
					</div>
					<div class="panel-body">
					    ...
					    <div id="files" class="files">
                            <?php
                                if(isset($_POST['tumunu_sil'])){
                                    ...
                                    foreach($resim_sil as $deger){
                                        $resim_name = mysql_fetch_assoc(mysql_query("SELECT * FROM projeresimler WHERE id='$deger'"));
                                        $resim_name2 = "../".$resim_name['resim'];
                                        if(file_exists($resim_name2)){
                                            unlink("$resim_name2");
                                            mysql_query("DELETE FROM projeresimler WHERE id='$deger'");
                                        }
                                    }
                                }                            
                            ?>
					    	...
					    	<?php
					    		$resim = mysql_query("SELECT * FROM projeresimler WHERE resid='$id' ORDER BY ordernum ASC");
	...
	$(function () {
	    ...
	    var url = window.location.hostname === 'blueimp.github.io' ?
                '//jquery-file-upload.appspot.com/' : 'projedosyaYukle.php?id=<?php echo $id;?>';
