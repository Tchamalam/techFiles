				...
				<div class="panel panel-default">
					...
					if(isset($_POST['gonder'])){
						...
						if($_FILES["dosya"]["size"] > 0){
							...
							$sss = mysql_fetch_assoc(mysql_query("SELECT * FROM iletisim WHERE id='$id'"));
							$sss_adi = $sss["resim"];
							...
							if ($yukle){
								...
								$resim = mysql_query("SELECT * FROM iletisim WHERE id='$id'");
								...
								$guncelle = mysql_query("UPDATE iletisim SET baslik='$baslik', resim='$yeniresimadi', yazi='$yazi' WHERE id='$id' and dil='".$diller->id."'");
								}
							...
							$yazi = $_POST["yazi".$diller->links];
							$yazi=str_replace("'", "`", $yazi);
							$guncelle = mysql_query("UPDATE iletisim SET baslik='$baslik', yazi='$yazi' WHERE id='$id' and dil='".$diller->id."'");
							}
							if($guncelle){
					...
					$gelen = mysql_fetch_assoc(mysql_query("SELECT * FROM iletisim WHERE id='$id'"));
					...
					<form class="form-horizontal" method="POST" action="<?php $PHP_SELF?>" enctype="multipart/form-data">
						...
						<div id="myTabContent" class="tab-content">
							...
							$j++;
							  $dyr_kyt=mysql_fetch_object(mysql_query("select * from iletisim where id='$id' and dil='".$diller2->id."'"));?>
							<div class="tab-pane fade <?php if($j==1){ echo "active"; } ?> in" id="<?php echo $diller2->links?>">
								...
								<fieldset>
									...
									<div class="form-group">
										...
										<div class="col-lg-10">
										<input type="text" class="form-control" id="inputEmail" name="baslik<?php echo $diller2->links?>" placeholder="SSS Başlığı" value="<?php echo $dyr_kyt->baslik?>">
										</div>
									</div>
					...
					if($resimid > 0){
						...
						$resim = mysql_query("SELECT * FROM iletisimresimler WHERE id='$resimid'");
						...
						$sil = mysql_query("DELETE FROM iletisimresimler WHERE id='$resimid'");
					...
					<div class="row">
				    	...
				    	$rs = mysql_query("SELECT * FROM iletisimresimler WHERE resid='$id' ORDER BY ordernum ASC");
				        ...
				        <div class="col-xs-3" id="listItem_<?php echo $rs_getir['id']; ?>">
				            ...
				            <center><a href="iletisimduzenle.php?id=<?php echo $id?>&resimid=<?php echo $rs_getir['id']?>"><span class="glyphicon glyphicon-remove"></span></a></center>
		...
		$(function() {
		  ...
		  $("#test-list").sortable({
		    ...
		    update:function(){
		      ...
		      $.post("iletisimresimsiralaislem.php",posted,function(e){
