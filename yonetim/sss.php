		...
		<div class="row">
			...
			<div class="col-md-9">
				<div class="panel panel-default">
					...
					<h3 class="panel-title">Satış Noktası</h3>
					</div>
					...
					if($id > 0){
						...
						$dyr = mysql_fetch_assoc(mysql_query("SELECT * FROM sss WHERE id='$id'"));
						$dyr_adi = $dyr["resim"];
						if(file_exists("../$dyr_adi")){
							@unlink("../$dyr_adi");
						}

						$sil = mysql_query("DELETE FROM sss WHERE id='$id'");
						
						if($sil){

							$resimc = mysql_query("SELECT * FROM sssresimler WHERE resid='$id'");
							while ($resim_res = mysql_fetch_array($resimc)) {
								$resim = $resim_res["resim"];
								unlink("../$resim");
							}

							$sil = mysql_query("DELETE FROM sssresimler WHERE resid='$id'");
					...
					?>
						...
						<div class="col-md-4"><b><u>İşlemler</u></b></div>
						  ...
						  $sayfa = @intval($_GET['s']);
		                  if(!$sayfa) $sayfa = 1;
		                  $toplam = mysql_num_rows(mysql_query("SELECT * FROM sss group by id"));
		                  $limit = 10;
		                  $sayfa_sayisi = ceil($toplam/$limit);
		                  if($sayfa > $sayfa_sayisi) $sayfa = 1;
		                  $goster = $sayfa * $limit - $limit;

						  $pr = mysql_query("SELECT * FROM sss group by id ORDER BY ordernum ASC LIMIT $goster,$limit");
						  ...
						  <div class="col-md-4"><a href="sssduzenle.php?id=<?php echo $pr_getir['id']?>">Düzenle</a> - <a href="sss.php?id=<?php echo $pr_getir['id']?>" onClick="return confirm('Bu Kayıtı Silmek İstediğinizden Emin misiniz?')">Sil</a></div>
						  </li>
						  ...
						  <ul class="pagination">
		                    ...
		                    if($sayfa > 1){
		                        $onceki = $sayfa - 1;
		                        echo "<li><a href='sss.php?s=$onceki' class'prev'><i class='fa fa-chevron-left'></i></a></li>";
		                    }else{
		                    ...
		                    for($i= $sayfa - $gorunen; $i < $sayfa + $gorunen + 1; $i++){
		                        ...
		                        if($i > 0 and $i <= $sayfa_sayisi){
		                                ...
		                                echo "<li><a href='sss.php?s=$i'>$i</a></li>";
		                            }
		                        }
		                    }
		                    ...
		                    if($sayfa != $sayfa_sayisi){
		                        $sonraki = $sayfa +1;
		                        echo "<li><a href='sss.php?s=$sonraki' class='next'><i class='fa fa-chevron-right'></i></a></li>";
		                    }else{
		...
		$(function() {
		  $("#test-list").sortable({
		    update:function(){
		      ...
		      $.post("ssssiralaislem.php",posted,function(e){
