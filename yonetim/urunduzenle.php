			...
			<div class="col-md-9">
				...
				<div class="panel panel-default">
					...
					if(isset($_POST['gonder'])){
						...
						mysql_query("DELETE FROM `urunozellikler` WHERE urunid='".$id."'");
						...
						if($_FILES["dosya"]["size"] > 0){
							...
							if ($yukle){
								...
								$ekle =  mysql_query("UPDATE urunler SET katid='$kat', renkid='$renk', baslik='$baslik', baslikseo='$baslikseo', public='$public', resim='$yeniresimadi', kisa_yazi='$kisa_yazi', yazi='$yazi', yazi2='$yazi2', title='$title', description='$description', keywords='$keywords' WHERE id='$id' and dil='".$diller->id."'");
								}


								if($ekle){
								if(!empty($_POST['ozellik'])) {
							    foreach($_POST['ozellik'] as $ozellik) {
							    	mysql_query("INSERT INTO `urunozellikler`(`ozellikid`, `urunid`) VALUES ('".$ozellik."','".$id."')");
								    }
								}
							...
							while($diller=mysql_fetch_object($sql_dil)){
								...
								$ekle2 = mysql_query("UPDATE urunler SET katid='$kat', renkid='$renk', public='$public', baslik='$baslik', baslikseo='$baslikseo', kisa_yazi='$kisa_yazi', yazi='$yazi', yazi2='$yazi2', title='$title', description='$description', keywords='$keywords' WHERE id='$id' and dil='".$diller->id."'");
								}
					...
					$gelen = mysql_fetch_assoc(mysql_query("SELECT * FROM urunler WHERE id='$id'"));

					$resid = mysql_real_escape_string($_GET['resid']);

					if($resid > 0):

						$resim = mysql_fetch_assoc(mysql_query("SELECT * FROM urunler WHERE id='$resid'"));
						$delimg = $resim['resim'];
						if(file_exists("../$delimg")){
							unlink("../$delimg");
							mysql_query("UPDATE urunler SET resim='' WHERE id='$resid'");
							header("Location:urunduzenle.php?id=".$resid);
						}

					endif;
					if($pdfid > 0):

						$pdfs = mysql_fetch_assoc(mysql_query("SELECT * FROM urunler WHERE id='$pdfid'"));
						$delpdf = $pdfs['pdf'];
						if(file_exists("../$delpdf")){
							unlink("../$delpdf");
							mysql_query("UPDATE urunler SET pdf='' WHERE id='$pdfid'");
							header("Location:urunduzenle.php?id=".$pdfid);
						}
					...
					<form class="form-horizontal" method="POST" action="<?php $PHP_SELF?>" enctype="multipart/form-data">
						...
						<div id="myTabContent" class="tab-content">
                            ...
                            <div class="form-group">
                                ...
                                <a href="urunduzenle.php?resid=<?php echo $id;?>" class="col-lg-3">Resmi Sil</a>
                                </div>
                            </div>									
                            <?php endif; ?>
							...
							while($diller2=mysql_fetch_object($sql_dil2)){
							  $syf_kyt=mysql_fetch_object(mysql_query("select * from urunler where id='$id' and dil='".$diller2->id."'"));
							$j++;
							?>
							<div class="tab-pane <?php if($j==1){ echo "active"; } ?> fade in" id="<?php echo $diller2->links?>">
								...
								<fieldset>
									...
									<div class="form-group">
										<div class="col-lg-12">
											<button name="gonder" type="submit" class="btn btn-default">Ürün Düzenle</button>
										</div>
									</div>
